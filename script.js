...
async function fetchImageForPlace(placeName) {
    ...
    const data = await response.json();
    return data.photos.length > 0 ? data.photos[0].src.medium : 'https://via.placeholder.com/100'; // Fallback if no image found
}
...
function displayWeatherInfo(weatherInfo) {
    ...
    const listItem = document.createElement('li');  // Add weather info as a list item
    listItem.innerHTML = `
        <div class="place-container">
    ...
    const placesList = document.getElementById('places-list');
    placesList.appendChild(listItem);  // Append weather info to the places list
}
